...
export async function fetchGitHubRepos() {
  const r = await fetch(apiUrl("/api/repos/github"), { credentials: "include" });
  if (!r.ok) throw new Error("Failed to load GitHub repos");
  return r.json();
}

export async function fetchRepos() {
  const r = await fetch(apiUrl("/api/repos"), { credentials: "include" });
  if (!r.ok) throw new Error("Failed to load repos");
  return r.json();
}

export async function registerRepo(full_name) {
  const r = await fetch(apiUrl("/api/repos/register"), {
    method: "POST",
    credentials: "include",
...
export async function removeRepo(id) {
  const r = await fetch(apiUrl(`/api/repos/${id}`), { method: "DELETE", credentials: "include" });
  const data = await r.json();
